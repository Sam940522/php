<html>
    ...
    <?php
        ...
        {   
            ...
            $result=mysqli_query($conn, "select * from user"); //藉由變數conn查詢user資料表的資料並將結果儲存至變數result
            while ($row=mysqli_fetch_array($result)) {  //呼叫變數result的資料並藉while迴圈逐筆存入變數row
                ...
                /*
                將row的資料逐列輸出至表格
                每列分別為 修改 的超連結(跳到19.user_edit_form.php)(點選後將這欄位的id存到變數row)
                刪除 的超連結(跳到17.user_delete.php)(點選後將這欄位的id存到變數row)id欄位的資料pwd欄位的資料
                */
            }
            echo "</table>"; //結束表格
        }
