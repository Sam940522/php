<?php
    ...
    {    
     ...
     $sql="insert into user(id,pwd) values('{$_POST['id']}', '{$_POST['pwd']}')"; //將變數id及變數pwd 存入user表格的id欄位及pwd欄位
     echo $sql; //輸出 insert into user(id,pwd) values('變數id的值', '變數pwd的值')
     if (!mysqli_query($conn, $sql)) {  //如果mysqli_query($conn, $sql)執行失敗 執行下行
      echo "新增命令錯誤"; //輸出 新增命令錯誤
    }
    else //如果mysqli_query($conn, $sql)執行成功 執行下行
    {  
     echo "新增使用者成功，三秒鐘後回到網頁"; //輸出 新增使用者成功，三秒後回到網頁
