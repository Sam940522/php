<?php
  ...
  if ($login==TRUE) { //如果變數login內容為 TURE,執行下行
    session_start(); //使用session保存使用者資料
    $_SESSION["id"]=$_POST["id"]; //儲存接收到的資料id
    echo "登入成功"; //輸出 登入成功
    echo "<meta http-equiv=REFRESH content='3, url=11.bulletin.php'>"; //上行代碼執行完經過3秒後,自動跳轉到11.bulletin.php
  }
  else{
    echo "帳號/密碼 錯誤"; //輸出 帳號/密碼 錯誤
