<?php
    ...
    {   
        ...
        /*執行mysqli_query用以偵錯 使用接收到的bid欄位尋找在變數conn中的bid欄位 並更新title欄位為接收到的title 更新content欄位為接收到的content
        更新time欄位為接收到的time 更新type欄位為接收到的type 以上發生錯誤執行下行*/

            echo "修改錯誤"; //輸出 修改錯誤
            echo "<meta http-equiv=REFRESH content='3, url=11.bulletin.php'>"; //3秒後跳轉到11.bulletin.php
        }
        else
        {
            echo "修改成功，三秒鐘後回到佈告欄列表"; //輸出 修改成功，三秒鐘後回到佈告欄列表
            echo "<meta http-equiv=REFRESH content='3, url=11.bulletin.php'>"; //3秒後跳轉到11.bulletin.php
        }
    }
